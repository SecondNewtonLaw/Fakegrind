...
IMAGE_IMPORT_DESCRIPTOR *Fakegrind::IATHookService::GetImportTable(HMODULE hModule) {
    const auto lpDOS = reinterpret_cast<IMAGE_DOS_HEADER *>(hModule);
    ASSERT(lpDOS->e_magic == IMAGE_DOS_SIGNATURE);
    const auto lpNT = reinterpret_cast<IMAGE_NT_HEADERS *>(lpDOS + lpDOS->e_lfanew);
...
void Fakegrind::IATHookService::Initialize() { /* No Initialization required*/ }

void Fakegrind::IATHookService::HookByOrdinal(unsigned short ordinal, void *replaceWith) {
...
void Fakegrind::IATHookService::HookByName(const char *name, void *replaceWith) {
    auto lpLogger = ServiceManager::GetSingleton().GetService<Services::LoggerService>();
    auto hModule = GetModuleHandle(nullptr);
    auto pImportDescriptor = this->GetImportTable(hModule);
    while (pImportDescriptor->Name != 0) {
...
void Fakegrind::IATHookService::Uninitialize() {
